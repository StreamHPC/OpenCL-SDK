...
private:
    ...
    void mouseDrag(const sf::Event::MouseMoveEvent& event);    // Handle mouse dragging
    void mouseWheel(const sf::Event::MouseWheelEvent& event);  // Handle mouse wheel movement
...
void NBody::initializeGL()
{
    ...
    glUseProgram(gl_program); checkError("glUseProgram(gl_program)");
    for(auto vbo_vao : {
        std::make_pair(&gl_pos_mass.front, &vertex_array.front),
        std::make_pair(&gl_pos_mass.back,  &vertex_array.back)})
    {
        glGenBuffers(1, vbo_vao.first); checkError("glGenBuffers(1, &vertex_buffer)");
        glBindBuffer(GL_ARRAY_BUFFER, *vbo_vao.first); checkError("glBindBuffer(GL_ARRAY_BUFFER, vertex_buffer)");
            glBufferData(GL_ARRAY_BUFFER, pos_mass.size() * sizeof(cl_float4), pos_mass.data(), GL_STATIC_DRAW); checkError("glBufferData(GL_ARRAY_BUFFER, quad.size() * sizeof(float), quad.data(), GL_STATIC_DRAW)");
        glBindBuffer(GL_ARRAY_BUFFER, 0); checkError("glBindBuffer(GL_ARRAY_BUFFER, 0)");

        glGenVertexArrays(1, vbo_vao.second); checkError("glGenVertexArrays(1, &vertex_array)");
        glBindVertexArray(*vbo_vao.second); checkError("glBindVertexArray(vertex_array)");
            glBindBuffer(GL_ARRAY_BUFFER, *vbo_vao.first);
            glVertexAttribPointer(0, 3, GL_FLOAT, GL_FALSE, sizeof(cl_float4), (GLvoid *)(NULL));                     checkError("glVertexAttribPointer(0, 3, GL_FLOAT, GL_FALSE, sizeof(cl_float4), (GLvoid *)(NULL))");
            glVertexAttribPointer(1, 1, GL_FLOAT, GL_FALSE, sizeof(cl_float4), (GLvoid *)(NULL + 3 * sizeof(float))); checkError("glVertexAttribPointer(1, 1, GL_FLOAT, GL_FALSE, sizeof(cl_float4), (GLvoid *)(NULL + 3 * sizeof(float)))");
    ...
    glClearColor(0.f, 0.f, 0.f, 1.f); checkError("glClearColor(0.f, 0.f, 0.f, 0.f)");
    glEnable(GL_DEPTH_TEST); checkError("glDisable(GL_DEPTH_TEST)");
    glDepthFunc(GL_LESS);    checkError("glDepthFunc(GL_LESS)");
    glDisable(GL_CULL_FACE); checkError("glDisable(GL_CULL_FACE)");
    glPointSize(1.5f);
}
...
void NBody::initializeCL()
{
    ...
    velocity_buffer = cl::Buffer{ opencl_context, CL_MEM_READ_WRITE, particle_count * sizeof(cl_float3), nullptr };
    queue.enqueueFillBuffer(velocity_buffer, cl_float4{ 0, 0, 0, 0 }, 0, particle_count * sizeof(cl_float4));
    queue.finish();

    // Translate OpenGL object handles into OpenCL handles
    cl_pos_mass.front = cl::BufferGL{opencl_context, CL_MEM_READ_WRITE, gl_pos_mass.front};
    cl_pos_mass.back  = cl::BufferGL{opencl_context, CL_MEM_READ_WRITE, gl_pos_mass.back};

    // Translate
    interop_resources = cl::vector<cl::Memory>{ cl_pos_mass.front, cl_pos_mass.back };
}

void NBody::updateScene()
{
    auto nbody = cl::KernelFunctor<cl::BufferGL, cl::BufferGL, cl::Buffer, cl_uint, cl_float>{cl_program, "nbody"};
    ...
    nbody(
        ...
        cl_pos_mass.back,
        velocity_buffer,
        static_cast<cl_uint>(particle_count),
        0.0001f
    );
...
void NBody::render()
{
    glClear(GL_COLOR_BUFFER_BIT | GL_DEPTH_BUFFER_BIT); checkError("glClear(GL_COLOR_BUFFER_BIT)");

    glUseProgram(gl_program); checkError("glUseProgram(gl_program)");
    glBindVertexArray(vertex_array.front); checkError("glBindVertexArray(vertex_array)");
    glBindBuffer(GL_ARRAY_BUFFER, gl_pos_mass.front); checkError("glBindBuffer(GL_ARRAY_BUFFER, gl_pos_mass.front)");

    if (needMatrixReset) setMatrices();

    glDrawArrays(GL_POINTS, 0, static_cast<GLsizei>(particle_count)); checkError("glDrawArrays(GL_POINTS, 0, static_cast<GLsizei>(particle_count))");
...
void NBody::event(const sf::Event& event)
{
    ...
    case sf::Event::MouseWheelMoved:
        dist += (float)sqrt(pow((double)x_abs_range,2)+pow((double)y_abs_range,2)) * 1.1f * event.mouseWheel.delta * (-0.2f);
        dist = abs(dist);
...
void NBody::setMatrices()
{
    ...
    glm::mat4 matWorld = glm::rotate(
        glm::rotate(
            ...
            theta,
            glm::vec3{ 1, 0, 0 }
        ), // theta rotates around z-axis
        phi,
        glm::vec3{ 0, 0, 1 } // theta rotates around z-axis
    );
    ...
    glm::mat4 matProj = glm::perspective<float>(
        fov,
        static_cast<float>(getSize().x) / getSize().y,
        0.001f,
        1000000.0f
    );

    auto loc_MVP = glGetUniformLocation(gl_program, "mat_MVP"); checkError("glGetUniformLocation(gl_program, \"mat_MVP\");");
    glUniformMatrix4fv(loc_MVP, 1, GL_FALSE, glm::value_ptr(matProj * matView * matWorld)); checkError("glUniformMatrix4fv(loc_MVP, 1, GL_FALSE, glm::value_ptr(matProj * matView * matWorld));");
